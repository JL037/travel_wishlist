...
@router.post("/user/saved-cities", response_model=CityOut)
async def save_city(city: CityCreate, db: AsyncSession = Depends(get_db), current_user: User = Depends(get_current_user)):
    new_city = UserSavedCity(user_id=current_user.id, city=city.city)
    db.add(new_city)
    await db.commit()
    await db.refresh(new_city)
    return {"id": new_city.id, "city": new_city.city, "message": "City saved!"}

@router.get("/user/saved-cities", response_model=list[CityOut])
async def get_saved_cities(db: AsyncSession = Depends(get_db), current_user: User = Depends(get_current_user)):
...
@router.delete("/user/saved-cities/{city_id}")
